...
Prereqs:
  1. ~/.kaggle/kaggle.json (kaggle.com -> Settings -> API -> Create New Token).
  2. evallab/kaggle/.env with HF_TOKEN=hf_... (gitignored; token of an account
     that accepted the gated issai/* datasets).
...
def read_hf_token() -> str:
    env = HERE / ".env"
    if not env.exists():
        sys.exit(f"missing {env} (expected HF_TOKEN=...)")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        tmpdir = Path(tmp)
        (tmpdir / "hf_token.txt").write_text(token + "\n")
        ...
        created = run(["kaggle", "datasets", "create", "-p", str(tmpdir)])
    ...
    pushed = run(["kaggle", "kernels", "push", "-p", str(HERE)])
